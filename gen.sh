...
if [[ -z "$session_cookie" ]]; then
    echo \[FATAL\] session_cookie not set
    exit 1
fi
...
if [ "$day" -gt "$num_days_of_aoc" ]; then
    echo \[FATAL\] day "$day" geq the number of aoc days "$num_days_of_aoc"
    exit 3
fi
...
if [[ ! -f "$txt" ]]; then
    echo \[FATAL\] "failed to curl input"
    exit 4
fi

mv "$txt" ../input/ && cp template.py "$py"
if [ $? -eq 0 ]; then
    echo \[INFO\] generated "$py" and "$txt"
else
    echo \[FATAL\] something happened while generating "$py" and "$txt"
    exit 5
fi
